import { mkdirSync, writeFileSync, symlinkSync, existsSync, readlinkSync, unlinkSync } from 'fs';
...
export function runInstallSkill() {
  ...
  const agentsDir = join(cwd, '.agents', 'skills', 'botmem-cli');
  const claudeDir = join(cwd, '.claude', 'skills', 'botmem-cli');
  const agentsSkillFile = join(agentsDir, 'SKILL.md');
  const claudeSkillFile = join(claudeDir, 'SKILL.md');

  // 1. Write to .agents/skills/botmem-cli/SKILL.md
  mkdirSync(agentsDir, { recursive: true });
  writeFileSync(agentsSkillFile, SKILL_CONTENT);
  console.log(`Wrote skill → ${agentsSkillFile}`);

  // 2. Symlink .claude/skills/botmem-cli/SKILL.md → .agents copy
  mkdirSync(claudeDir, { recursive: true });

  if (existsSync(claudeSkillFile)) {
    // Check if it's already a symlink pointing to the right place
    try {
      const target = readlinkSync(claudeSkillFile);
      const expected = relative(claudeDir, agentsSkillFile);
      if (target === expected || target === agentsSkillFile) {
        console.log(`Symlink already exists → ${claudeSkillFile}`);
        console.log('\nDone. The botmem-cli skill is now available.');
        return;
      }
    } catch {
      // Not a symlink, remove and recreate
    }
    unlinkSync(claudeSkillFile);
  }

  const relPath = relative(claudeDir, agentsSkillFile);
  symlinkSync(relPath, claudeSkillFile);
  console.log(`Linked skill → ${claudeSkillFile} → ${relPath}`);

  console.log('\nDone. The botmem-cli skill is now available.');
  console.log('Use /botmem-cli in Claude Code to access it.');
}
