...
3. **Embedding** each memory as a vector for semantic search via PostgreSQL search index
...
```
                        ...
                        +--------+---------+
                          /      |       \
               +---------+  +---+---+  +--+--------+
               |PostgreSQL|  | Redis |  | PostgreSQL search index |
...
| Search   | PostgreSQL search index (hybrid BM25 + vector search, cosine similarity) |
