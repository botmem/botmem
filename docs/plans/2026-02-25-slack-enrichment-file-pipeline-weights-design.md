...
The `weights` column in the memories table has defaults `{semantic:0:0, recency:0, importance:0.5, trust:0.5, final:0}`. The enrich processor never updates it. Search computes weights live but only returns a single `score`. List view shows raw DB defaults — hence all zeros in the UI.
...
weights = {semantic: 0: 0, recency, importance, trust, final: 0}
```
