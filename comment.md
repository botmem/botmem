...
**Comment:** Frontend built with React 19, Vite 6, TypeScript (ES2022 target). No Flash, Shockwave, ActiveX, Silverlight, NACL, or Java applets used anywhere in the application.
...
**Comment:** All first-party scripts bundled with Vite (content-hashed filenames providing implicit integrity). External scripts restricted by Content-Security-Policy to specific trusted domains: apis.google.com, _.firebaseio.com, _.googleapis.com, static.cloudflareinsights.com. No untrusted third-party code loaded. Docker images built from pinned dependencies via pnpm lockfile.
...
**Comment:** NestJS ThrottlerModule configured globally (100 requests per 60 seconds). Endpoint-specific rate limits: registration (3/min), login (5/min), password reset (3/min), recovery key submission (5/min), CLI auth approval (5/min). Cloudflare provides additional DDoS protection at the edge.
...
**Comment:** Minimum password length enforced at 12 characters via @MinLength(12) class-validator decorator in RegisterDto, ChangePasswordDto, and ResetPasswordDto. Frontend validates on signup and password reset forms with client-side length check and HTML minLength attribute.
...
**Comment:** Password reset tokens (email-based out-of-band verification) expire in 60 minutes — industry standard for email delivery latency. Tokens are cryptographically random (256-bit), single-use, stored as SHA-256 hashes in the database. Expiry validated server-side before acceptance. CLI auth sessions expire in 300 seconds (5 minutes).
...
**Comment:** localStorage stores only UI preferences (theme selection) and memory bank IDs (non-sensitive). Authentication primarily uses httpOnly, secure, sameSite=strict cookies for session management. No passwords, API keys, or PII stored in browser storage. Zustand state rehydrates from server-validated session cookies.
...
**Comment:** PostHog tracks authentication events (user_logged_in, user_registered, password_changed). Job/sync logs record all connector data access operations with timestamps and progress tracking. Connector sync history maintained in jobs table with status, errors, and completion metrics. Recovery key submission events logged (without key content).
