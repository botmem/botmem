 ...
 * relabel-unknown, queue status, search index info, stats, dedup.
 */
...
describe('Memory CRUD & Misc (SRCH-064 → SRCH-080)', () => {
  ...
  it('SRCH-064 get memory by ID returns full memory', async () => {
    ...
    const listRes = await authedRequest(user.accessToken).get('/api/memories?limit=1');

    if (listRes.body.items && listRes.body.items.length > 0) {
      const memId = listRes.body.items[0].id;
      const res = await authedRequest(user.accessToken).get(`/api/memories/${memId}`);
      ...
      const res = await authedRequest(user.accessToken).get(
  ...
  it('SRCH-065 get memory with unknown ID returns null or 404', async () => {
    const res = await authedRequest(user.accessToken).get('/api/memories/nonexistent-id-12345');
  ...
  it('SRCH-066 delete memory removes it', async () => {
    ...
    if (listRes.body.items && listRes.body.items.length > 0) {
      const memId = listRes.body.items[0].id;
      const res = await authedRequest(user.accessToken).delete(`/api/memories/${memId}`);

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ ok: true });

      // Verify it's gone
      const check = await authedRequest(user.accessToken).get(`/api/memories/${memId}`);
      ...
      const res = await authedRequest(user.accessToken).delete(
        '/api/memories/00000000-0000-0000-0000-000000000000',
      );
  ...
  it('SRCH-067 delete same memory twice — second fails or is no-op', async () => {
    ...
    const res1 = await authedRequest(user.accessToken).delete(`/api/memories/${fakeId}`);

    // First delete on non-existent may succeed (no-op) or 404
    const res2 = await authedRequest(user.accessToken).delete(`/api/memories/${fakeId}`);
  ...
  it('SRCH-068 pin memory sets pinned true', async () => {
    ...
    if (listRes.body.items && listRes.body.items.length > 0) {
      const memId = listRes.body.items[0].id;
      const res = await authedRequest(user.accessToken).post(`/api/memories/${memId}/pin`);

      expect(res.status).toBe(201);
      expect(res.body).toEqual({ ok: true });
    } else {
      // No memories — verify endpoint returns ok structure
      const res = await authedRequest(user.accessToken).post('/api/memories/fake-id/pin');
  ...
  it('SRCH-069 unpin memory sets pinned false', async () => {
    ...
    if (listRes.body.items && listRes.body.items.length > 0) {
      ...
      // Pin first
      await authedRequest(user.accessToken).post(`/api/memories/${memId}/pin`);

      // Then unpin
      const res = await authedRequest(user.accessToken).delete(`/api/memories/${memId}/pin`);

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ ok: true });
    } else {
      const res = await authedRequest(user.accessToken).delete('/api/memories/fake-id/pin');
  ...
  it('SRCH-070 recall memory increments recallCount', async () => {
    ...
    if (listRes.body.items && listRes.body.items.length > 0) {
      const memId = listRes.body.items[0].id;
      const res = await authedRequest(user.accessToken).post(`/api/memories/${memId}/recall`);

      expect(res.status).toBe(201);
      expect(res.body).toEqual({ ok: true });
    } else {
      const res = await authedRequest(user.accessToken).post('/api/memories/fake-id/recall');
  ...
  it('SRCH-071 related memories returns linked memories by graph', async () => {
    ...
    if (listRes.body.items && listRes.body.items.length > 0) {
      const memId = listRes.body.items[0].id;
      const res = await authedRequest(user.accessToken).get(`/api/memories/${memId}/related`);

      expect(res.status).toBe(200);
      // Should be an array (may be empty if no links)
      expect(Array.isArray(res.body) || res.body === null || typeof res.body === 'object').toBe(
        true,
      );
    } else {
      const res = await authedRequest(user.accessToken).get('/api/memories/fake-id/related');
  ...
  it('SRCH-072 thumbnail proxy returns image or not-found', async () => {
    const res = await authedRequest(user.accessToken).get('/api/memories/fake-id/thumbnail');
  ...
  it('SRCH-073 thumbnail with DEK cold returns 503 encrypted error', async () => {
    ...
    const res = await authedRequest(coldUser.accessToken).get('/api/memories/fake-id/thumbnail');
  ...
  it('SRCH-074 retry-failed endpoint re-enqueues failed memories', async () => {
    const res = await authedRequest(user.accessToken).post('/api/memories/retry-failed');
  ...
  it('SRCH-075 retry-failed with limit param caps re-queued count', async () => {
    const res = await authedRequest(user.accessToken).post('/api/memories/retry-failed?limit=5');
  ...
  it('SRCH-076 relabel-unknown updates WhatsApp Unknown labels', async () => {
    const res = await authedRequest(user.accessToken).post('/api/memories/relabel-unknown');
  ...
  it('SRCH-077 queue status returns BullMQ depths for all queues', async () => {
    const res = await authedRequest(user.accessToken).get('/api/memories/queue-status');
  ...
  // SRCH-078: Search index info endpoint
  it('SRCH-078 search index info returns metadata', async () => {
    const res = await authedRequest(user.accessToken).get('/api/memories/search-index-info');

    expect(res.status).toBe(200);
    // Should return collection info from the PostgreSQL search index.
  ...
  it('SRCH-079 memory dedup prevents duplicate sourceId + connectorType', async () => {
    ...
    const res = await authedRequest(user.accessToken).get('/api/memories/stats');
