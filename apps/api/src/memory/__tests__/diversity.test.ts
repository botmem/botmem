...
/**
 * Test the private diversifyResults method via (service as any).
 * We construct MemoryService with minimal mocks — only the diversification logic is exercised.
 */
...
describe('diversifyResults', () => {
  ...
  ) => (service as any).diversifyResults(candidates, limit, diversityFactor);

  it('returns empty for empty candidates', () => {
    expect(diversify([], 10)).toEqual([]);
  });

  it('returns single candidate as-is', () => {
    const c = makeCand('a', 0.9, 'gmail');
    const result = diversify([c], 5);
  ...
  it('returns top-K by score when all same connector', () => {
    ...
    const result = diversify(candidates, 3);
  ...
  it('interleaves two connectors with close scores', () => {
    ...
    const result = diversify(candidates, 4, 0.15);
  ...
  it('relevance wins when one connector is far below threshold', () => {
    ...
    const result = diversify(candidates, 3, 0.15);
  ...
  it('round-robins across three connectors', () => {
    ...
    const result = diversify(candidates, 6, 0.15);
  ...
  it('diversityFactor=0 yields pure score sort', () => {
    ...
    const result = diversify(candidates, 4, 0);
  ...
  it('diversityFactor=1 aggressively diversifies', () => {
    ...
    const result = diversify(candidates, 4, 1);
    expect(result[0].id).toBe('g1');
