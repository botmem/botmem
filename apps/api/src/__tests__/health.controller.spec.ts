...
describe('HealthController', () => {
  ...
  let mockPgSearchService: { healthCheck: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    controller = new HealthController(mockDbService, mockPgSearchService, mockConfigService);
  });
  ...
  it('returns all services connected when everything is healthy', async () => {
    ...
    expect(result.services.searchIndex.connected).toBe(true);
  });
  ...
  it('returns 200 with all services down (never throws)', async () => {
    ...
    mockPgSearchService.healthCheck.mockResolvedValue(false);
    mockPing.mockRejectedValue(new Error('Connection refused'));
    ...
    expect(result.services.redis.connected).toBe(false);
    expect(result.services.searchIndex.connected).toBe(false);
  });

  it('PgSearchService.healthCheck returning false yields connected: false', async () => {
    mockPing.mockResolvedValue('PONG');
    mockPgSearchService.healthCheck.mockResolvedValue(false);
