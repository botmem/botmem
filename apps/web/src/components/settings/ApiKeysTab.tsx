...
export function ApiKeysTab() {
  ...
  const activeKeys = visibleKeys.filter((k) => !isExpired(k.expiresAt));
          ...
          {visibleKeys.map((key) => {
            ...
            return (
              <div
                key={key.id}
                className={[
                  'flex items-center justify-between border-3 border-nb-border p-3 bg-nb-surface-muted',
                  expired ? 'opacity-60' : '',
                ].join(' ')}
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {expired && (
                      <span className="font-mono text-[11px] font-bold uppercase tracking-wider bg-nb-orange text-black px-1.5 py-0.5 border-2 border-nb-border">
                        EXPIRED
                      </span>
                    )}
                  </div>
